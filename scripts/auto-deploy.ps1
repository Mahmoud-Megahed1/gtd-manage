...
if (-not (Test-Path $sshKey)) {
    ...
    Write-Success "?? ????? SSH Key"
    
    Write-Host "`n" -NoNewline
    Write-Host "???????????????????????????????????????????????????????" -ForegroundColor Yellow
    Write-Info "??? ????? SSH Public Key ??? Hostinger!"
    Write-Host "???????????????????????????????????????????????????????" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "1. ????: https://hpanel.hostinger.com" -ForegroundColor White
    Write-Host "2. ???? ???: Advanced > SSH Keys" -ForegroundColor White
    Write-Host "3. ????: Add New Key" -ForegroundColor White
...
if (-not $SkipBuild) {
    ...
    Write-Step "3/8 - ???? ?????? (--SkipBuild)"
}

# Verify build
if (-not (Test-Path "dist/index.js")) {
    Write-Error "dist/index.js ??? ?????!"
    exit 1
}
...
try {
    ...
    Write-Host "  - SSH Key ???? ?? Hostinger Panel" -ForegroundColor Gray
    exit 1
}
...
Write-Host "   • ??? .deploy-config.json ????? ??? ??????? ?????" -ForegroundColor Yellow
Write-Host "   • ?? ????? ??? Git (????? ?? .gitignore)" -ForegroundColor Yellow
Write-Host ""
Write-Host "???????????????????????????????????????????????????????????????" -ForegroundColor Gray
Write-Host ""
Write-Host "?? ?????? ??????! ??" -ForegroundColor Green
