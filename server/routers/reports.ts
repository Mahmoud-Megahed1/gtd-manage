...
export const reportsRouter = router({
  summary: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (!conn) {
        ...
        const salesRows = demo.list("sales").filter((r: any) => within(r.saleDate) && r.status === 'completed' && !r.invoiceId);
        ...
        return {
          ...
          installmentsTotal: paidInstallmentsTotal,
          manualSalesTotal,
          net: paidInvoicesTotal + paidInstallmentsTotal + manualSalesTotal
      ...
      const instSum = await conn.select({ total: sql<number>`COALESCE(SUM(${installments.amount}), 0)` })
        .from(installments)
        .where(and(...instWhere));

      // Sales (Manual) - completed only
      ...
      const salesSum = await conn.select({ total: sql<number>`COALESCE(SUM(${sales.amount}), 0)` })
        .from(sales)
  ...
  timeseries: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (!conn) {
        ...
        return range.map(r => {
          const v = acc[r.key];
          const net = v.invoices + v.installments + v.sales - v.expenses - v.purchases;
  ...
  timeseriesBreakdown: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const initExp: Record<string, number> = Object.fromEntries(expenseStatuses.map(s => [s, 0]));
